 ...
 * strings such as person names. The score is normalized such that 0 equates to
 * no similarity and 1 is an exact match.
 * 
 * The Jaro distance metric states that given two strings s1 and s2 , their
 * distance is d':
 * 
 ...
 * transpositions
 * 
 * Each character of is compared with all its matching characters in . The
 * number of matching (but different sequence order) characters divided by two
 * defines the number of transpositions.
 * 
 * Jaro-Winkler distance uses a prefix scale which gives more favourable ratings
 * to strings that match from the beginning for a set prefix length . Given two
 * strings s1 and s2, their Jaro-Winkler distance d'' is:
 * 
 * d'' = d' + (l*p*(1-d'))
 * 
 * where: d': is the Jaro distance for strings and l : is the length of common
 * prefix at the start of the string up to a maximum of 4 characters p : is a
 * constant scaling factor for how much the score is adjusted upwards for having
 * common prefixes. The standard value for this constant in Winkler's work is
 * 
...
public class JaroWinklerDistance extends AbstractStringSimilarity {
	...
	public double getDistance(String source, String target) {
		...
		for (int index = 0; index < sourceLength; index++) {
			...
			/**
			 * if(!sourceMatch[index]) { for(int tempIndex =
			 * start;tempIndex<end;tempIndex++) { if(source.charAt(index) ==
			 * target.charAt(tempIndex)) { if(index != tempIndex) {
			 * transposeCandidate++; break; } } } }
			 **/
		}
